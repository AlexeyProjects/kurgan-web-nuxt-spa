                        ...
                        <div class="viewing-body-content">
                            ...
                            <div class="review-panel">
                                <div class="btn-address">
                                    ...
                                    {{ choosedSight.polygon[0].lat }} , {{ choosedSight.polygon[0].lon }}
                                </div><br>
...
<script>
	...
	export default {
		...
		computed: {
            ...
            getWorkDaysString() {
				var newListDays = []
				this.choosedSight.availabilities.forEach(element => {
					var stringDay = ''
					switch(element.day) {
						case 0 :
							stringDay = 'Вс'
					...
					element.dayString = stringDay
					if ( element.enable ) {
						newListDays.push(stringDay)
					}
				});
				return newListDays.join(',')
			},
			getFirstDay() {
				var startDay = ``
				this.choosedSight.availabilities.forEach((element) => {
					if ( element.enable ) {
						startDay = `${element.start} - ${element.end}`
					}
				})
				return startDay
			}
